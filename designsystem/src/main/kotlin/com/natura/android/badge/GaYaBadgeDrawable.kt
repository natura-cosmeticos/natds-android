...
class GaYaBadgeDrawable(
    ...
    fun updateColorBadgeDrawable(color: Int) {
        this.color = color
        invalidateSelf()
    }
    ...
    private fun drawText(canvas: Canvas, x: Float, y: Float) {
        canvas.drawText(
            if (this@GaYaBadgeDrawable.count > getMaxValue() && limit != UNLIMITED) getMaxValueLabel() else count.toString(),
            x,
            y,
            mTextPaint
        )
